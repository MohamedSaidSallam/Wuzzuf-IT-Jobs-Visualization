...
def makeJobsCSV():
    with open(f"{OUTPUT_FOLDER}/jobs.csv", "w", newline='',encoding='utf-8') as jobsCSV:
        ...
        csvWriter.writerow(["Key", "Title","Company","City","PostedDate","ExperienceNeeded","CareerLevel","JobType","Salary","Vacancies","Education Level", "About The Job", "Job Requirements"])
                ...
                if os.path.splitext(filename)[1][1:] == 'json':
                    with open(f"{JOBS_FOLDER}/{filename}","r") as job:
                        ...
                        for keyword in jobData["Keywords"]:
                            csvWriter.writerow([f"{jobData['title'] + jobData['Company']}", keyword])
                ...
                if os.path.splitext(filename)[1][1:] == 'json':
                    with open(f"{JOBS_FOLDER}/{filename}","r") as job:
                        ...
                        for jobRole in jobData["Job Roles"]:
                            csvWriter.writerow([f"{jobData['title'] + jobData['Company']}", jobRole])
